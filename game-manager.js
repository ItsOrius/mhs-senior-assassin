...
async function confirmUser(email, firstName, lastName, color, avatarUrl) {
  ...
  const { data, error } = await supabase
    ...
    .upsert([
      { email, first_name: firstName, last_name: lastName, avatar_url: avatarUrl ?? null, favorite_color: color }
    ]);
  if (error) {
    console.error(error);
  } else {
